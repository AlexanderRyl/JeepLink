...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof L === 'undefined') {
        ...
        if (statusText) {
            statusText.innerHTML =
                '<span class="dot" style="background:orange"></span> Offline Mode';
        }
    ...
    try {
        // UPDATED: Coordinates for SLU Maryheights, Bakakeng
        const baguioCoords = [16.3831, 120.5901];
        ...
        // UPDATED: Marker Text for SLU
        const mainStation = L.marker(baguioCoords).addTo(map);
        mainStation.bindPopup('<b>JeepLink HQ</b><br>SLU Maryheights Campus').openPopup();
    } catch (error) {
